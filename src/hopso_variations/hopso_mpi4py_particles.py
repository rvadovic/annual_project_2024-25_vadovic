...
from hamiltonian_ansatz_cost_functions.costF_2q_IvaH2_qiskit import objective_function_1
from hopso_result_analyser_tools import get_all_final_energies
...
def hopso(cost_fn, hp, num_particles, runs, dimension, max_cut, e_min, vectors, vel_mag, gbest, max_iterations):
    ...
    while iteration < max_iterations:
        ...
        if np.any(cos_theta < -1) or np.any(cos_theta > 1) or np.isnan(cos_theta).any():
            particle_position = np.random.uniform(r, r + 2 * np.pi)
            cos_theta = (particle_position - attractor) / A
...
# s=1, with 500-iter, 10p means ==B = 5000 => (5K/10P)-1 = 500**-1

hp = [1, 1, 2*np.pi, 10]
maxcut = 4.10             # A_threshold
...
hopso(objective_function_1, hp, num_particles, 1, 6, maxcut, e_min, vectors, vel_mag, gbest, max_iterations)
end_time = perf_counter()
print("" + str(end_time - start_time))
print(get_all_final_energies)